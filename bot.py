...
async def check_agent(update: Update, context: ContextTypes.DEFAULT_TYPE, current_state: int) -> int | None:
    ...
    # Кнопки зарплатного флоу — никогда не агент (ищем подстроки, т.к. есть эмодзи)
    salary_buttons_substrings = [
        "январь", "февраль", "март", "апрель", "май", "июнь",
        "июль", "август", "сентябрь", "октябрь", "ноябрь", "декабрь",
        "назад", "другой месяц", "другой год", "всё верно", "есть вопросы",
    ]
    for sub in salary_buttons_substrings:
        if sub in text_lower:
            return None
    # Года (2024-2030)
    if text.isdigit() and 2020 <= int(text) <= 2030:
        return None
