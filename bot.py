...
Данные из Google Sheets через CSV. Python-telegram-bot v20+.
...
PROXY_URL = os.environ.get("BOT_PROXY", "")  # на PythonAnywhere не задаём → без прокси; дома задай в переменных BOT_PROXY=socks5://127.0.0.1:10808

# GID вкладок
PASSWORDS_GID = 836125341  # лист «Пароли»
...
def resolve_department(name: str) -> tuple[str | None, int | None]:
    """Находит ключ в SHEETS по полному или частичному названию отдела.
    Возвращает (полное_название, gid) или (None, None)."""
    # Точное совпадение
    if name in SHEETS:
        return name, SHEETS[name]
    # Частичное: «Разработка» → «Разработка (менеджеры)»
...
def find_salary(department: str, fio: str, month: int, year: int) -> dict | None:
    """Ищет расшифровку в CSV нужного отдела по ФИО, месяцу и году."""
...
def get_short_name(entry: dict) -> str:
    """Имя для обращения: Обращение → имя из ФИО → коллега."""
...
def format_response(department: str, data: dict, entry: dict) -> str:
    ...
    dept = department  # алиас для краткости
...
def year_keyboard() -> list[list[str]]:
    ...
    return [[str(y) for y in range(now - 2, now + 1)]]  # 2024, 2025, 2026 (текущий + 2 назад)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Предзагрузка паролей
    try:
        passwords = load_passwords()
        context.bot_data["passwords"] = passwords
    except Exception as e:
        logging.error("Не загрузились пароли: %s", e)
        await update.message.reply_text(
            "Не могу загрузить данные. Попробуй через минуту или напиши Лизе @elizavetkkaa16 🙏",
...
async def get_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    passwords: dict = context.bot_data.get("passwords", {})
...
async def get_month(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data is None:
        # Диагностика: показываем что ищем и что есть в CSV
        debug_info = (
            f"🔍 *Что ищу:*\n"
        ...
        try:
            ...
            if gid is not None:
                df = load_csv(gid)
                # Какие месяцы и ФИО есть в таблице
...
async def handle_after_view(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Кнопки после просмотра расшифровки."""
...
async def handle_month_or_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Роутер для состояния MONTH: месяц → расшифровка, кнопка → действие."""
...
def main():
    ...
    # Режим: вебхук (Render) или поллинг (домашний комп)
    ...
    if render_url:
        ...
        # Polling-режим для домашнего компа
        logging.info("Бот запущен через %s", proxy_label)
        app.run_polling()
