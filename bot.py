...
async def router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    ...
    # Если пользователь уже авторизован или в процессе ввода пароля — не трогаем
    if context.user_data.get("entry") or context.user_data.get("in_password_flow"):
        return None

    # Цифры (4-20 символов) — похоже на пароль, пропускаем в ConversationHandler
    if text.isdigit() and 4 <= len(text) <= 20:
        return None

    agent = classify_question(text)

    if agent == "komplektator":
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data["in_password_flow"] = True
    await update.message.reply_text(
...
async def get_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        context.user_data.pop("in_password_flow", None)
        return ConversationHandler.END
